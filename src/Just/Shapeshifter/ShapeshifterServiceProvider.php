...
class ShapeshifterServiceProvider extends ServiceProvider
{
	...
	private function bindFacadeModels ()
	{
		$this->app['ss'] = $this->app->share(function ($app)
		{
			return new Fronteer\FronteerHelper(new Fronteer\Metatag\MetaTagGenerator);
		});
	}

	private function registerAliasses ()
	{
		AliasLoader::getInstance()->alias('SS', 'Just\Shapeshifter\Facades\SS');
